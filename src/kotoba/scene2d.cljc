(ns kotoba.scene2d
  "A whole 2D frame as GPU data — assembles the sky gradient params + every quad (entity sprites via
   kotoba.sprite2d.layout with the per-entity idle breathing/bob, fx particles + 7-segment text) so the
   WebGL2/WebGPU runtime draws the frame the way kotoba.sprite2d/draw-2d! did on Canvas2D, but through
  ...
  (:require [kami.sprite-gpu :as sg]  ;; canonical impl now lives in kotoba-lang/webgpu; see README.md
            [kami.text :as txt]  ;; ditto -- kotoba-lang/text is an unrelated string-utils lib
...
(defn fx->quads
  "Screen-space juice → quads: particles are circles; floating text is 7-segment glyphs. fx are placed
   relative to the screen centre [cx cy] (kotoba.sprite2d's convention)."
  [[cx cy] fx]
  (into []
